...
--
-- HTTP server module
-- this module is based on 'https://github.com/nodemcu/nodemcu-firmware/blob/dev/lua_modules/http/http.lua' by 'Vladimir Dronnikov' aka 'dvv'
...
do
	...
	local http_handler = function(handler)
		return function(conn)
			...
			-- header parser
			local cnt_len = 0
			local onheader = function(conn, name, value)
				-- TODO: look for Content-Type: header
				-- to help parse body
				-- parse content length to know body length
				if name == "content-length" then
					cnt_len = tonumber(value)
				elseif name == "expect" and value == "100-continue" then
